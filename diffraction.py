...
def _d_over_devo(expt_name):
    """Compute d/d_evo for a named experiment from YAML files."""
    with open('experiments.yaml') as f:
        expts = yaml.safe_load(f)
    with open('materials.yaml') as f:
        materials = yaml.safe_load(f)
    e = expts[expt_name]
    v   = float(materials[e['material']]['speed_of_sound'])
    d   = float(e['thickness'])
    w   = float(e['waist'])
    tau = float(e['pulse_duration'])
    # Quoted durations are FWHM; convert to the model 1/e half-duration
    tau /= 2 * np.sqrt(np.log(2))
    return 4 * d * v * tau / w**2


def create_diffraction_figure(d_over_devo=1.0, plot_acceleration=True, normalization='global', N=10**5):
    """
    Create diffraction comparison figure using full numerical integration.
    ...
    normalization : str
        'global' (single colorscale), 'per-panel', or 'per-column'.
    """
    ...
    t_nat_values = [1.5, 1.2, 0.9, 0.6, 0.3]  # Late to early (top to bottom)

    field_type = "acceleration" if plot_acceleration else "velocity"
    print(f"Full numerical integration diffraction comparison (5×3 grid)")
    ...
    for row, t_nat in enumerate(t_nat_values):
        ...
        for col, eta in enumerate(eta_values):
            print(f"  η = {eta:.2g}: computing...", end='', flush=True)

            # For given eta and d_evo, compute w and tau
            # d_evo = w²/(4*v*tau)
            # eta = w/(2*v*tau)
            # Solving: w = 2*d_evo/eta, tau = d_evo/(v*eta²)
            w_phys = 2 * d_evo_ref / eta
    ...
    # Compute colorscale
    # all_data is indexed as [row * n_cols + col]
    n_rows = len(t_nat_values)
    n_cols = len(eta_values)
    if normalization == 'per-panel':
        print(f"\nUsing per-panel normalization")
        vmaxes = [np.abs(data).max() for data in all_data]
    elif normalization == 'per-column':
        print(f"\nUsing per-column normalization")
        col_vmaxes = []
        for col in range(n_cols):
            col_data = [all_data[row * n_cols + col] for row in range(n_rows)]
            col_vmaxes.append(np.abs(np.array(col_data)).max())
        print(f"  Column vmaxes: {[f'{v:.3e}' for v in col_vmaxes]}")
        vmaxes = [col_vmaxes[row_col % n_cols] for row_col in range(len(all_data))]
    else:
    ...
    for row, t_nat in enumerate(t_nat_values):
        for col, eta in enumerate(eta_values):
            ...
            if col == 2:
                ratio = _d_over_devo('Northup')
                ax.axhline(y=ratio, color='black', linestyle='--', linewidth=1.2, alpha=0.7)

            # Labels and titles
            if col == 0:
                ax.set_ylabel(r'$z/d_{\mathrm{evo}}$', fontsize=24)
            else:
                ax.set_yticklabels([])

            if row == len(t_nat_values) - 1:  # Bottom row
                ax.set_xlabel(r'$r/d_{\mathrm{evo}}$', fontsize=24)
            else:
                ax.set_xticklabels([])

            if row == 0:  # Top row
                ax.set_title(f'η = {eta:.2g}', fontsize=24, pad=8)
    ...
    if plot_acceleration:
        cbar_label = r'$a/(S_0 \xi v)$' if normalization == 'global' else r'$a/a_{\mathrm{max}}$'
    else:
        cbar_label = r'$u$ [nat. units]' if normalization == 'global' else r'$u/u_{\mathrm{max}}$'
...
Examples:
  ...
  python diffraction.py --norm per-column           # Per-column normalization
  ...
  python diffraction.py 2.5 --velocity -N 1000000  # Combined options
    ...
    parser.add_argument('--norm', choices=['global', 'per-panel', 'per-column'],
                        default='global', help='Normalization mode (default: global)')
